...
@section('og:description')
    {{ $upazela->description ?? 'সর্বশেষ খবর, বিশ্লেষণ এবং প্রতিবেদন পড়ুন আমাদের পোর্টালে।' }}
@endsection
            ...
            <div class="row">
                <div class="col-lg-8 col-md-8">
                    <div class="rachive-info-cats">
                        <a href="{{route('home')}}"><i class="las la-home"> </i> </a>  <i class="las la-angle-right"></i> {{$upazela->name}}
                    </div>
{{--                    @if($dristrict->upazilas->count() > 0)--}}
{{--                        <div class="subcategory-scroll mt-3">--}}
{{--                            @foreach($dristrict->upazilas as $upzela)--}}
{{--                                <a href="{{route('sub-category-news',[$upzela->id,$upzela->slug])}}">--}}
{{--                                    {{ $upzela->name }}--}}
{{--                                </a>--}}
{{--                            @endforeach--}}
{{--                        </div>--}}
{{--                    @endif--}}
                    ...
                    <div class="archivePage-content2">
                        <div class="row">
                            ...
                            @foreach($upazelasnewses as $upazelasnews)
                                <div class="themesBazar-3 themesBazar-m2">
                                    <div class="archivePage-wrpp2">
                                        <div class="archive2-image">
                                            <a href="{{route('news-detail',$upazelasnews->id)}}">
                                            <img class="lazyload" src="{{asset($webLogo->lazyload_logo)}}" data-src="{{asset($upazelasnews->image)}}" alt="{{$upazelasnews->title}}" title="{{$upazelasnews->title}}">
                                            </a>
                                        </div>
                                        <h4 class="archivePage2-title">
                                            <a href="{{route('news-detail',[$upazelasnews->id])}}">{{$upazelasnews->title}} </a>
                                        </h4>

                                        <div class="archive-meta2">
                                            <a href="#"><i class="las la-tags"> </i>
                                                {{ \Carbon\Carbon::parse($upazelasnews->created_at)->locale('bn')->bnDiffForHumans() }}
                            ...
                            <div class="d-flex justify-content-center mt-2 mb-2 pt-2 pb-2 border-top">
                                {{ $upazelasnewses->links('pagination::bootstrap-5') }}
                            </div>
